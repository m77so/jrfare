...
fs.writeFileSync(path.join(outputDir,"stations.tsv"),output.stations.map(s => [s.id,s.name,s.kana].join("\t")).join("\n"))

fs.writeFileSync(path.join(outputDir,"lines.tsv"),output.lines.map(s => [s.id,s.name,s.kana].join("\t")).join("\n"))

let distanceArr = []
for(var l of output.lines) {
  for(var i=0;i<l.stationIds.length;++i){
